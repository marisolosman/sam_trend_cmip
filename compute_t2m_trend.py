...
trends = np.zeros_like(ds1_exp.tas[0,:])
pval = np.zeros_like(ds1_exp.tas[0,:])

for i in range(ds1_exp.tas.shape[1]):
    y = ds1_exp.tas[:, i]
...
for i in range(4):
 #   ax = fig.add_subplot(1,5,i+1)
     ...
     #titulo general
     plt.suptitle('DJFMAM TAS Trend (K/30year)', fontsize=12, x=0.52, y=0.85)
...
plt.savefig(RUTA + 'djfmam_1979_present_tas_trends.png', dpi=300, bbox_inches='tight', orientation='landscape', 
            papertype='A4')
