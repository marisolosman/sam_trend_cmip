...
for i in range(len(EXPERIMENT)):
    ...
    ARCHIVO = VAR + '_A' + FREQ + '_' + MODEL[0] + '_' + EXPERIMENT[i] +'_' + PERIOD[i] + '.nc4'
    FILE = glob.glob(PATH + ARCHIVO)
    #Open data with xarray
    ds = xr.open_dataset(FILE[0], decode_coords=False)#, decode_cf=False)
...
#
trends = np.zeros_like(ds1_exp.psl[0,:])
pval = np.zeros_like(ds1_exp.psl[0,:])
#
for i in range(ds1_exp.psl.shape[1]):
    y = ds1_exp.psl[:, i]
    [trends[i], interc, r_va, pval[i], z] = stats.linregress(np.arange(len(ds1_exp.time.values)), y)
trends = np.reshape(trends, (len(EXPERIMENT), len(ds_exp['lat']), len(ds_exp['lon'])))
pval = np.reshape(pval, (len(EXPERIMENT), len(ds_exp['lat']), len(ds_exp['lon'])))
...
for i in range(len(EXPERIMENT)):
     ...
     #titulo general
     plt.suptitle(MODEL[0] + ' DJFMAM SLP Trend (hPa/30year)', fontsize=12, x=0.52, y=0.85)
...
plt.savefig(RUTA + MODEL[0] + '_djfmam_1979_present_'+ VAR + '_trends.png', dpi=300,
